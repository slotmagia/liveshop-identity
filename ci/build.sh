...
docker buildx build --pull --push   --build-context "kernel=$workspace_root/kernel-go"   --build-context "protocol=$CI_PROJECT_DIR/protocol"   --build-context "platform-protocol=$workspace_root/liveshop-platform/protocol"   --file "$CI_PROJECT_DIR/business/backend/deploy/Dockerfile"   --tag "$CI_REGISTRY_IMAGE/backend:$tag"   "$CI_PROJECT_DIR/business/backend"
