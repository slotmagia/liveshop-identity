...
本目录是 `admin` surface 的前端贡献，产物类型是 **iframe**，由 `admin` Host 加载。
...
- 套餐、角色等权限集合必须使用 Host 表单的 `kind: 'checkbox-tree'`，树叶提交稳定权限码，父节点支持整组选择和半选；禁止 textarea 手工权限码。
